      ...
      <div className="bg-zinc-100 flex flex-col justify-center sm-block">
        <form className="max-w-[400px] w-full mx-auto bg-zinc-100 p-8 px-8 roundend-lg" onSubmit={handleSubmit}>
          ...
          <div className='flex justify-between text-gray-400 py-2'>
            <p className='flex-items-center'><input className="mr-2" type="checkbox" />Beni Hatırla</p>
            <a href="#"><p>Şifreni mi unuttun?</p></a>
          </div>
          <div className='flex justify-between text-gray-400 py-2'>
            <p>Hesabınız yok mu?  <Link to={"/register/"}><u>Kayıt olun..</u></Link></p>
          </div>
          <div>
            <button className="w-full my-5 py-2 bg-green-500 shadow-lg hover:shadow-teal-500/60 font-semibold rounded-lg" type='submit' >Oturum Aç</button>
          </div>
        </form>
      </div>
    </div>
